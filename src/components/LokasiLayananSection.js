...
const LokasiLayananSection = () => {
  ...
  const [heroData, setHeroData] = useState({
    Title1: '50+',
    Subtitle1: 'Mitra Kami',
    Title2: '100+',
    Subtitle2: 'Kecamatan terjangkau',
  });
  ...
  useEffect(() => {
    ...
    // Fetch location data
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/lokasi-layanan-sections`)
      .then((response) => {
    ...
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/lokasi-layanan-hero-section`)
      .then((response) => {
  ...
  const groupLocations = (locations) => {
    ...
    locations.forEach((location) => {
      const foundGroup = groupedCircles.find(
        (group) =>
          calculateDistance(
            group.center[0],
            group.center[1],
      ...
      <div className="lokasi-header" data-aos="fade-down">
        ...
        <div className="lokasi-subtext" data-aos="fade-up" data-aos-delay="200">
          <div className="subtext-item">
            <span className="subtext-number">{heroData.Title1}</span>
            <span className="subtext-text">{heroData.Subtitle1}</span>
          </div>
          <div className="subtext-divider">|</div>
          <div className="subtext-item">
            <span className="subtext-number">{heroData.Title2}</span>
            <span className="subtext-text">{heroData.Subtitle2}</span>
          </div>
        </div>
      </div>
